...
us_df = us_df[columns_of_interest].dropna()

@app.route("/", methods=["GET", "POST"])
def index():
    # Default start and end dates
    start_date = us_df['date'].min()
    end_date = us_df['date'].max()
    ...
    filtered_df = us_df[(us_df['date'] >= start_date) & (us_df['date'] <= end_date)]
...
def generate_bar_chart(data):
    ...
    plt.title("Total People Vaccinated Over Time in the USA")
    plt.xlabel("Date")
